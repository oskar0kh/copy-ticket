...
Phase 7  라운드 관리 및 데이터 정리
    ↓
Phase 8  Redis 대기열/진입 제한 + 트래픽 테스트
```
...
## 8. Phase 5 — 좌석 선택 + 예매 완료 + Redis 선점, TTL 적용
...
| 5-11 | TTL 만료 좌석 자동 복구 배치 | 1초 주기로 LOCKED 좌석 중 hold_expires_at <= now() 인 것을 AVAILABLE로 자동 복구 (스케줄러 실행) |
...
## 11. Phase 8 — Redis 대기열/진입 제한 + 트래픽 테스트

**목표:** 예매 진입량을 제한하고, 고동시성 환경에서 중복 예매 없이 TTL 복구와 확정 정합성이 유지되는지 검증한다.

| 순서 | 작업 | 설명 |
|------|------|------|
| 8-1 | Redis 대기열/진입 제한 | "예매하기" 버튼 클릭 → Redis Queue에 요청 저장, 토큰 기반 진입량 제한(라운드당 예: 1000명 동시 진입) |
| 8-2 | 트래픽 테스트 | 다중 사용자 고동시성 요청 시뮬레이션 — 중복 예매 0건, TTL 복구 동작, hold_expires_at 추적, 확정 정합성 검증 |

**산출물:** Redis 대기열/진입 제한 로직, 고동시성 트래픽 테스트 코드
